...
Version 1.0 | First Created Nov 4, 2024 | Updated Nov 8, 2024
...
The tabulation of the dependent variable, DRINKING_D, shows that the majority of crashes do not involve alcohol impairment. Specifically, 94.3% of crashes (proportion = 0.943) are categorized as not involving drunk driving, while only 5.7% of crashes (proportion = 0.057) involve a drunk driver. This indicates that alcohol-related crashes constitute a relatively small proportion of the overall crash data. The skewed distribution of the dependent variable suggests that drunk driving incidents are relatively rare compared to non-alcohol-related crashes, which is an important consideration for model performance and evaluation, as it may impact the model's ability to accurately predict alcohol involvement.
...
The Chi-Square test results indicate significant associations between most of the binary predictors and alcohol involvement in crashes, with the exception of CELL_PHONE. Specifically, FATAL_OR_M (crashes resulting in fatality or major injury) showed a highly significant association, with a Chi-Square statistic of 167.56 and a p-value of 0.000, suggesting that severe crashes are more likely to involve alcohol. Similarly, OVERTURNED (crashes involving an overturned vehicle) also had a significant association (Chi-Square = 122.79, p = 0.000), indicating that overturned vehicles are more frequently associated with alcohol impairment.

CELL_PHONE, however, did not show a significant relationship with alcohol involvement, as evidenced by a Chi-Square statistic of 0.16 and a p-value of 0.687. This lack of significance implies that cell phone use during a crash does not have a meaningful association with whether alcohol was involved, which may be due to the risk dynamics associated with cell phone use being distinct from those of alcohol impairment.

On the other hand, SPEEDING exhibited a very strong association with alcohol involvement, with a Chi-Square statistic of 376.78 and a p-value of 0.000. This finding highlights the increased likelihood of speeding when alcohol is involved, reflecting the reduced inhibitions and impaired judgment associated with alcohol consumption. AGGRESSIVE driving also had a significant association (Chi-Square = 67.60, p = 0.000), suggesting that alcohol impairment might exacerbate aggressive driving behaviors.

Age-related predictors also revealed significant associations. DRIVER1617 (involving a driver aged 16 or 17) had a significant Chi-Square statistic of 20.45 (p = 0.000), indicating that crashes involving young drivers are less likely to involve alcohol, possibly because of stricter legal consequences and lower rates of alcohol consumption among this age group. Conversely, DRIVER65PLUS (involving a driver aged over 65) had a Chi-Square statistic of 80.60 (p = 0.000), demonstrating a significant inverse relationship. This may be because older drivers are generally less likely to engage in alcohol-impaired driving, perhaps due to greater awareness of the risks and more conservative driving behavior.

Overall, the significant associations observed for most predictors highlight important behavioral and demographic factors linked to alcohol involvement in crashes. These findings suggest that specific risky driving behaviors, such as speeding and aggressive driving, are more prevalent among alcohol-impaired drivers, while age plays a critical role in the likelihood of alcohol involvement, with young and older drivers showing distinct patterns. The results underscore the need for targeted interventions focusing on these high-risk factors to reduce alcohol-related crashes.
...
The logistic regression analysis examining factors associated with alcohol involvement in crashes provided several significant findings. The model includes a range of predictors, and each coefficient reflects the change in log-odds of alcohol involvement when the corresponding variable increases by one unit, holding other variables constant.

The intercept was highly significant, indicating a baseline log-odds of alcohol involvement when all predictors are zero. The variable FATAL_OR_M, representing crashes involving fatalities or major injuries, had a positive and highly significant coefficient (Estimate = 0.814,p<0.0001), suggesting that such crashes are strongly associated with an increased likelihood of alcohol involvement. Similarly, OVERTURNED had a positive and significant association (Estimate = 0.929, p<0.0001), indicating that crashes involving an overturned vehicle are more likely to involve alcohol-impaired drivers.

The coefficient for CELL_PHONE use was not statistically significant (Estimate = 0.030, p=0.881), implying no meaningful relationship between cell phone use and alcohol involvement in crashes. On the other hand, SPEEDING exhibited a large positive and highly significant coefficient (Estimate = 1.539, p<0.0001), highlighting a strong association between speeding and alcohol impairment. AGGRESSIVE driving, in contrast, had a significant negative coefficient (Estimate = -0.597, p<0.0001), suggesting that aggressive driving behaviors are less likely to be associated with alcohol involvement.

Age-related variables, DRIVER1617 and DRIVER65PLUS, both showed significant negative associations. DRIVER1617 (Estimate = -1.280, p<0.0001) indicated that crashes involving drivers aged 16-17 were significantly less likely to involve alcohol. Likewise, DRIVER65PLUS (Estimate = -0.775, p<0.0001) suggested that crashes involving drivers aged 65 or older also had a lower likelihood of alcohol involvement.

The socio-economic predictors, PCTBACHMOR (percentage of people with a bachelor’s degree or higher) and MEDHHINC (median household income), showed differing results. PCTBACHMOR was not significant (Estimate = -0.000371,p=0.775), indicating no strong relationship with alcohol involvement. In contrast, MEDHHINC had a small but significant positive effect (Estimate = 0.00000280, p=0.036), suggesting a slight association between higher median income and increased odds of alcohol involvement in crashes.

Overall, the model’s residual deviance (18340) and AIC (18360) indicate a reasonably good fit, and the significant predictors provide valuable insights into the factors associated with alcohol-impaired driving.
...
```
The table below summarizes the results of a logistic regression model predicting the likelihood of alcohol involvement in crashes. Each row represents a predictor variable, with corresponding estimates, standard errors, z-values, p-values, odds ratios (OR), and the 95% confidence interval (ranging from the lower bound, 2.5%, to the upper bound, 97.5%).

The intercept estimate is -2.733, with a highly significant p-value, suggesting a low baseline probability of alcohol involvement when all predictors are held constant. The odds ratio (OR) for the intercept is 0.065, indicating a low likelihood of alcohol involvement at the baseline.

FATAL_OR_M, which indicates whether a crash involved a fatality or major injury, has a positive and highly significant estimate of 0.814. The corresponding OR is 2.257, with a 95% confidence interval of 1.910 to 2.653, suggesting that such crashes are more than twice as likely to involve alcohol compared to non-severe crashes.

The OVERTURNED variable also has a positive and significant estimate of 0.929, with an OR of 2.532, indicating a strong association between overturned vehicles and alcohol involvement. The confidence interval (2.035 to 3.122) supports the robustness of this effect.

CELL_PHONE use is not significant (p = 0.881), with an OR of 1.030 and a wide confidence interval (0.684 to 1.488), suggesting no meaningful association between cell phone use and alcohol involvement in crashes.

SPEEDING has a highly significant positive estimate of 1.539, and the OR of 4.660 indicates that crashes involving speeding are more than four times as likely to involve alcohol. The confidence interval (3.974 to 5.450) further emphasizes this strong relationship.

Conversely, AGGRESSIVE driving has a significant negative estimate of -0.597, with an OR of 0.551, implying a reduced likelihood of alcohol involvement. The confidence interval (0.501 to 0.604) confirms the reliability of this inverse association.

Both age-related predictors, DRIVER1617 and DRIVER65PLUS, have significant negative estimates. DRIVER1617 has an OR of 0.278, indicating that crashes involving drivers aged 16-17 are much less likely to involve alcohol, with a confidence interval of 0.148 to 0.471. Similarly, DRIVER65PLUS has an OR of 0.461, suggesting a lower likelihood of alcohol involvement in crashes involving older drivers, with a confidence interval of 0.380 to 0.553.

The socio-economic variable PCTBACHMOR is not statistically significant (p = 0.775), and its OR of 1.000, with a narrow confidence interval, suggests no effect of educational attainment on alcohol involvement. However, MEDHHINC is significant (p = 0.036), but its OR of 1.000 and very narrow confidence interval imply a negligible effect of median household income on alcohol involvement in crashes.
...
In a couple sentences, recap what you did in the paper, and your findings.
Which variables are strong predictors of crashes that involve drunk driving? Which variables aren’t associated with the dependent variable?
Are the results surprising? Discuss, and mention whether the variables you expect to be significant actually are significant, and if so, whether the relationships with the dependent variable are in the direction you would
expect.
Is logistic regression appropriate here? In other words, would the modeling rare events methods proposed by Paul Allison be more
appropriate here?
Hint: look at the # and % of cases with values of ‘1’ for the dependent variable.
What are some limitations of the analysis? Discuss.
